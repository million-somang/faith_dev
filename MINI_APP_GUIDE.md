...
## 📷 마케팅용 미니앱 3단계 실화면 캡처 포인트 표준 선언 (필수 의무)

모든 미니앱은 관리자 페이지 및 SNS 자동 마케팅 카드뉴스 생성을 위해 **서로 다른 3단계 고유 화면(진입 ➡️ 조작 ➡️ 결과)**을 자동으로 캡처할 수 있도록 아래 HTML `data-screenshot-*` 표준 데이터 속성을 컴포넌트 JSX에 필수적으로 선언해야 합니다.

### 1단계: 진입 화면 (Entry)
- 앱 첫 접속 직후 순수 빈 폼 및 헤더가 보이는 상태입니다.
- 기본적으로 최상단 `window.scrollTo(0, 0)` 상태에서 자동 캡처됩니다.

### 2단계: 메인 컨텐츠 조작 화면 (Action)
- 사용자가 값을 입력하거나 주요 기능을 실행 중인 화면입니다.
- **클릭할 요소**: `<button data-screenshot-click="action">` 또는 `<div data-screenshot-click="action">`
- **입력할 폼**: `<input data-screenshot-input="테스트입력값" />` 또는 `<textarea data-screenshot-input="내용">`
- 예시: 평수 계산기의 '34평' 버튼, 지뢰찾기의 '보드 중앙 셀', 텍스트 검사기의 '샘플 문장 입력창'

### 3단계: 최종 결과 화면 (Result)
- 산출된 결과 리포트, 그래프, 스코어보드, 또는 결과 모달을 띄우는 화면입니다.
- **실행 버튼**: `<button data-screenshot-click="result">계산하기 / 결과보기</button>`
- **결과 영역 스크롤 타겟**: `<div data-screenshot-point="result">최종 산출 결과</div>`
- 예시: 퇴직금 계산기의 `[퇴직금 계산하기]` 버튼, 지뢰찾기의 `[명예의 전당]` 팝업 버튼, 계산기의 `=` 버튼
