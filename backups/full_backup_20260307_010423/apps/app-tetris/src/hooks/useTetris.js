...
export const useTetris = () => {
    ...
    const [nextPieceType, setNextPieceType] = useState(getRandomTetrominoType());
    ...
    const checkCollision = useCallback((piece, boardState, move = { row: 0, col: 0 }) => {
    ...
    const clearLines = useCallback((boardState) => {
        ...
        if (linesCleared > 0) {
            ...
            setScore(prev => {
                ...
                switch (linesCleared) {
                    case 1:
                        addedScore = SCORES.SINGLE_LINE;
                        break;
                    case 2:
                        addedScore = SCORES.DOUBLE_LINE;
                        break;
                    case 3:
                        addedScore = SCORES.TRIPLE_LINE;
                        break;
                    case 4:
                        addedScore = SCORES.TETRIS;
                        break;
                }
    ...
    const lockSpecificPiece = useCallback((pieceToLock) => {
        setBoard(prevBoard => {
    ...
    const movePiece = useCallback((rowOffset, colOffset) => {
    ...
    const gameLoop = useCallback((time) => {
        ...
        if (!isPaused && !isGameOver && deltaTime > getDropTime()) {
            movePiece(1, 0);
            lastUpdateTimeRef.current = time;
        }
        requestRef.current = requestAnimationFrame(gameLoop);
    }, [isPaused, isGameOver, getDropTime, movePiece]);
    useEffect(() => {
        requestRef.current = requestAnimationFrame(gameLoop);
        return () => {
