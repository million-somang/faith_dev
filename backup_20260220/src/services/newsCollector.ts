...
import { parseRSSXML, RSSItem } from '../utils/stockNewsCollector'
import { NewsCategory } from '../types/news.types'

const DB_PATH = process.env.DATABASE_PATH || './faith-portal.db'

// 카테고리별 RSS URL 설정
const CATEGORY_RSS_URLS: Record<NewsCategory, string> = {
...
export async function fetchAndSaveNews() {
    ...
    try {
        for (const [category, url] of Object.entries(CATEGORY_RSS_URLS)) {
            await processCategory(db, category as NewsCategory, url)
        }
    } catch (error) {
...
async function processCategory(db: Database.Database, category: NewsCategory, rssUrl: string) {
    try {
    ...
    `)
        ...
        const insertMany = db.transaction((newsItems: RSSItem[]) => {
            for (const item of newsItems) {
                ...
                const result = stmt.run(
                    category,
                    item.title,
                    stripHtml(item.summary), // 요약에서 HTML 태그 제거
                    item.link,
                    imageUrl, // thumbnail 컬럼이 아니라 image_url?? 아까 thumbnail로 바꿨는데...
                    item.source,
                    pubDate
                )
