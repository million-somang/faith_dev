...
utilsRoutes.get('/api/figma/file/:fileKey', async (c) => {
    ...
    if (!FIGMA_ACCESS_TOKEN)
        return c.json({ success: false, error: 'Figma Access Token이 설정되지 않았습니다.' }, 500);
    try {
        const response = await fetch(`https://api.figma.com/v1/files/${fileKey}`, { headers: { 'X-Figma-Token': FIGMA_ACCESS_TOKEN } });
        if (!response.ok)
            return c.json({ success: false, error: `Figma API 오류: ${response.status}`, details: await response.text() }, response.status);
        return c.json({ success: true, data: await response.json() });
    }
    catch (error) {
        return c.json({ success: false, error: error.message || '파일 가져오기 실패' }, 500);
    }
});
utilsRoutes.get('/api/figma/images/:fileKey', async (c) => {
    ...
    if (!FIGMA_ACCESS_TOKEN)
        return c.json({ success: false, error: 'Figma Access Token 미설정' }, 500);
    if (!nodeIds)
        return c.json({ success: false, error: 'Node IDs 필요' }, 400);
    try {
        ...
        return c.json({ success: false, error: error.message || '이미지 렌더링 실패' }, 500);
    }
});
utilsRoutes.get('/api/figma/styles/:fileKey', async (c) => {
    ...
    try {
        const response = await fetch(`https://api.figma.com/v1/files/${fileKey}`, { headers: { 'X-Figma-Token': FIGMA_ACCESS_TOKEN } });
        if (!response.ok)
            return c.json({ success: false, error: `Figma API 오류: ${response.status}` }, response.status);
        ...
        return c.json({ success: false, error: error.message || '스타일 가져오기 실패' }, 500);
    }
});
// ==================== Puppeteer API 연동 ====================
utilsRoutes.get('/api/puppeteer/screenshot', async (c) => {
    try {
        ...
        if (!token || token === 'demo_token_for_testing')
            return c.json({ success: false, error: 'BROWSERLESS_API_TOKEN not configured' }, 401);
        ...
        if (!response.ok)
            return c.json({ success: false, error: 'Browserless.io API error', details: await response.text() }, response.status);
