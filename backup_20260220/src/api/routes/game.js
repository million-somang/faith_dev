...
gameRoutes.post('/api/tetris/score', async (c) => {
    ...
    try {
        ...
        await DB.prepare('INSERT INTO tetris_scores (user_id, score) VALUES (?, ?)').bind(userId, score).run();
...
gameRoutes.get('/api/tetris/highscore/:userId', async (c) => {
    try {
        ...
        const DB = getDB(c);
        const highScore = await DB.prepare('SELECT MAX(score) as high_score FROM tetris_scores WHERE user_id = ?').bind(userId).first();
        return c.json({
            success: true,
...
gameRoutes.get('/api/sudoku/besttime/:userId/:difficulty', async (c) => {
    try {
        ...
        const difficulty = c.req.param('difficulty');
        const DB = getDB(c);
        const bestTime = await DB.prepare('SELECT MIN(time) as best_time FROM sudoku_scores WHERE user_id = ? AND difficulty = ?').bind(userId, difficulty).first();
        return c.json({
            success: true,
