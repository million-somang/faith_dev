...
newsRoutes.get('/api/news', async (c) => {
    ...
    try {
        ...
        if (includeStocks) {
            const newsWithStocks = await Promise.all(results.map(async (news) => {
...
newsRoutes.post('/api/news', async (c) => {
    try {
        ...
        const result = await DB.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, datetime('now'), ?, ?, ?, ?, ?, ?, ?)
        `).bind(category, title, summary || '', link, image_url || thumbnail || '', publisher || source || '', content || '', thumbnail || image_url || '', tagsJson || '[]', author || '', source || publisher || '', source_url || link, description || summary || '').run();
        return c.json({
            success: true,
...
newsRoutes.put('/api/news/:id', async (c) => {
    try {
        ...
        const result = await DB.prepare(`
            ...
            WHERE id = ?
        `).bind(category, title, summary || '', link, image_url || thumbnail || '', publisher || source || '', content || '', thumbnail || image_url || '', tagsJson, author || '', source || publisher || '', source_url || link, description || summary || '', id).run();
...
newsRoutes.post('/api/news/find-related-stocks', async (c) => {
    try {
        ...
        if (tickers.length === 0)
            return c.json({ success: true, stocks: [], message: 'No related stocks found' });
...
// 헬퍼 함수
async function parseGoogleNewsRSS(category = 'general') {
    const rssUrls = {
