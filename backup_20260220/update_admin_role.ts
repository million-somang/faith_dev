...
try {
    ...
    const user = db.prepare('SELECT id, email, role FROM users WHERE email = ?').get(userEmail) as any;
    if (!user) {
    ...
    // 3. Verify update
    const updatedUser = db.prepare('SELECT role FROM users WHERE email = ?').get(userEmail) as any;
