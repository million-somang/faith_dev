...
### 1.6 680px 이내 1화면 완결(Zero-Scroll) 컴팩트 최적화 표준 (모든 미니앱 공통 의무)

> **핵심 정의**: 팝업 크기가 850px로 지정되어 있더라도, **사용자가 실제로 조작하는 모든 핵심 인터랙션 영역(게임 보드, 계산기 키패드, 입력 폼, 실행 버튼)은 순수 세로 680px 이내에 100% 한눈에 들어와야 하며, 조작 도중 브라우저 스크롤바가 생겨 화면이 위아래로 밀리는 현상을 절대 허용하지 않는다.**

#### 1) 왜 680px 기준인가? (배경 및 기술적 이유)
- **실제 가용 브라우저 높이의 제약**:
  - 데스크톱 OS의 작업표시줄(40~50px), 브라우저 팝업 윈도우 상단 타이틀바/주소창/탭(40~80px), 사용자 OS의 디스플레이 배율(125%, 150%)이 적용되면, 850px 창 내부의 실제 렌더링 가용 높이는 **680px ~ 750px 내외**로 축소됩니다.
  - 만약 컴포넌트들을 방만하게 세로로 늘어놓아 총 높이가 750~850px에 도달하면, **화면 우측에 전체 스크롤바가 발생하여 하단 키패드(7, 8, 9, 0, 삭제)나 핵심 액션 버튼([투구], [계산하기], [복사])이 화면 밖으로 잘리게 됩니다.**
  - 사용자가 버튼을 누르기 위해 스크롤을 내리고, 결과를 보기 위해 다시 스크롤을 올리는 조작 경험은 플랫폼 품질을 크게 저해하므로 **모든 미니앱은 조작 모드에서 순수 680px 이내 1화면 완결을 의무 준수**해야 합니다.

#### 2) 5대 컴팩트 최적화 공통 원칙 (게임 / 계산기 / 유틸리티 공통)

```
+-------------------------------------------------------------+
| 1. 상단 바 (36px): 로고/타이틀 + 음소거(🔇) + 공유 버튼      |
| 2. 탭 네비게이션 (34px): [경기장/기능] [랭킹/내역] [가이드]  |
+-------------------------------------------------------------+
| 3. 원라인(One-Line) 통합 헤더 (44~48px)                     |
|    - 좌측: 프로필/구단명 [변경 ✏️] + 전적/등급 요약         |
|    - 우측: 상태/이닝 배지 + LED 카운트/인디케이터           |
+-------------------------------------------------------------+
| 4. 메인 비주얼/캔버스 (120~180px)                           |
|    - 그래픽 캔버스 슬림화 + 슬림 판정/피드백 인라인 배너    |
|    - ⚠️ 중복 입력 박스 일원화 (키패드와 중복 제거)          |
+-------------------------------------------------------------+
| 5. 고정 높이 2열(Grid) 히스토리 패널 (75~85px)              |
|    - ⚠️ 동적 세로 팽창 금지! 고정 높이 + 2열 그리드 배치    |
|    - 초과 데이터는 내부 스크롤 (외부 페이지 밀림 0px)       |
+-------------------------------------------------------------+
| 6. 인터랙션 키패드 / 액션 독 (200~210px)                    |
|    - 입력 슬롯 + 메인 액션 버튼 ([투구], [계산], [변환])   |
|    - 36~38px(h-9.5) 컴팩트 키패드 (1~9, 0, 초기화, 삭제)    |
+-------------------------------------------------------------+
| 🚀 총 높이: 약 650~675px -> 어떤 팝업창에서도 완벽한 1화면 노출 |
+-------------------------------------------------------------+
```

1. **인터랙션 중 동적 세로 팽창 원천 차단 (Zero Dynamic Layout Shift)**:
   - 회차(이닝)가 누적되거나, 계산 기록(History)이 쌓이거나, 옵션 태그가 추가될 때 **부모 컨테이너 높이가 세로로 늘어나는 것을 영구 금지**합니다.
   - 내역 표시 영역은 반드시 `h-[76px]` 또는 `h-[80px]`의 **고정 높이(Fixed Height)**를 지정하고, 공간 효율을 2배로 높이기 위해 **2열 그리드(`grid grid-cols-2 gap-1`)**로 배치합니다. 4개 이상의 데이터는 내부 스크롤(`overflow-y-auto custom-scrollbar`)로 소화하여 부모 레이아웃의 흔들림을 1px도 유발하지 않습니다.
2. **중복 입력 및 상태 표시 슬롯 과감한 일원화 (Eliminate Redundant Slots)**:
   - 상단 메인 캔버스나 전광판에 "입력값: ? ? ?"이 있고, 하단 키패드 상단에도 "선택 숫자: • • •"가 별도로 존재하는 것과 같은 **세로 공간 낭비성 중복 요소를 철저히 단일화**합니다.
3. **원라인(One-Line) 좌우 분할 통합 헤더**:
   - 프로필 카드와 LED 인디케이터/전광판이 상하 2단으로 나뉘어 90~100px을 차지하던 구조를 **좌측(구단/유저 정보) + 우측(상태/LED)**의 **단일 행(~48px)으로 압축**합니다.
4. **인터랙션 컨트롤러(키패드/버튼) 36~38px 컴팩트 표준**:
   - 숫자 키패드, 단위 선택 버튼, 기능 스위치는 `h-9.5` (38px, 모바일 기준 36~40px)로 규격화합니다. 터치 및 마우스 조작의 편안함을 유지하면서 4단 키패드가 팝업 최하단에 100% 여유 있게 안착되도록 합니다.
5. **최상위 뷰포트 잠금 (`h-screen max-h-[850px] overflow-hidden`)**:
   - 미니앱 최상위 `main` 컨테이너에 `overflow-hidden`을 선언하여 브라우저의 원치 않는 전체 세로 스크롤바 생성을 원천 차단합니다.

#### 3) 앱 유형별 680px 컴팩트 적용 가이드라인

| 앱 유형 | 680px 1화면 최적화 배치 구성 | 주요 적용 기법 |
|:---:|---|---|
...
### 4.2 그래픽 퀄리티 & 인터랙티브 모션 애니메이션 특화 가이드 (Visual & Motion Polish)

> 🎨 **최고급 그래픽 에셋 원칙 (Graphic Assets & Textures)**  
> 1. **평면(Flat) 디자인 탈피 & 3D 깊이감 부여**:  
>    - 밋밋한 1차원 평면 사각형을 피하고, 은은한 내부 음영(`inset 0 1px 2px rgba(255,255,255,0.8)`), 다층 드롭 섀도우(`shadow-sm`, `shadow-md`, `shadow-xl`), 입체 광택 셰이딩(방사형 그라데이션)을 적극 활용합니다.  
> 2. **질감과 텍스처 오버레이**:  
>    - 게임이나 도구의 테마에 맞는 맞춤형 텍스처(원목 결 무늬, 부드러운 펠트, 매끄러운 세라믹, 은은한 미세 격자 등)를 CSS 그라데이션과 SVG로 직접 코딩하여 독창적인 소프트웨어 감성을 완성합니다.  
> 3. **고해상도 커스텀 벡터(SVG) 아트워크**:  
>    - 저화질 픽셀 이미지 다운로드 대신, 깨짐 없이 선명한 인라인 SVG 컴포넌트(바둑판, 칩, 주사위, 보석, 카드, 그래프 등)를 자체 렌더링합니다.  

> ⚡ **살아 숨 쉬는 인터랙티브 모션 & 마이크로 인터랙션 (Motion Principles)**  
> 1. **탄성 있는 바운스 & 스프링 모션**:  
>    - 요소 등장, 타일 합체, 바둑돌 착수, 카드 뒤집기 시 `cubic-bezier(0.34, 1.56, 0.64, 1)` 계열의 스프링 탄성을 주어 물리적 실체가 느껴지는 쫀득한 손맛을 구현합니다.  
> 2. **사용자 액션 즉각 피드백**:  
>    - **클릭/터치**: `active:scale-95` 눌림 모션 및 파문(Ripple) 확산 효과.  
>    - **호버(Hover)**: 마우스 커서 위치에 따른 반투명 고스트 프리뷰(Ghost Stone/Preview) 및 좌표 툴팁.  
>    - **상태 강조**: 최근 조작/착수 지점의 지속적 펄스 링(Pulse Ring) 인디케이터.  
> 3. **성취감 극대화 축하 이펙트**:  
>    - 미션 클리어, 계산 완료, 5목 달성 시 황금빛 네온 빔, 글로우 레이저, 캔버스 콘페티(Confetti 폭죽) 파티클 애니메이션을 화려하게 연출합니다.  
> 4. **청각적 피드백 결합 (Web Audio API Procedural Audio)**:  
>    - 외부 MP3/WAV 파일 의존 없이 브라우저 내장 오실레이터로 경쾌한 버튼 클릭음, 청명한 원목 타격음, 승리 팡파르 아르페지오를 합성하여 모션과 완벽히 동기화합니다.  
