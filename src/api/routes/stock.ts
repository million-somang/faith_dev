...
interface YahooFinanceScreenerQuote {
    symbol: string;
    shortName?: string;
    longName?: string;
    regularMarketPrice: number;
...
interface YahooFinanceScreenerResponse {
    finance: {
        result?: {
            quotes: YahooFinanceScreenerQuote[];
        }[];
        error?: any;
    };
}

const stockRoutes = new Hono<{ Bindings: Bindings; Variables: Variables }>()
...
stockRoutes.get('/api/stock/quote/:symbol', async (c) => {
    ...
    try {
        ...
        return c.json({ success: false, message: 'Failed to fetch stock data', symbol, error: (error as Error).message }, 500)
    }
})
...
stockRoutes.get('/api/stocks/quotes', async (c) => {
    ...
    if (symbols.length === 0) return c.json({ success: false, message: 'No symbols provided' }, 400)
    try {
    ...
    } catch (error) { return c.json({ success: false, message: 'Failed to fetch stocks data' }, 500) }
})

// 미국 주요 주식 4대장
stockRoutes.get('/api/us-stocks/major', async (c) => {
    const symbols = ['AAPL', 'TSLA', 'NVDA', 'MSFT']
    const nameMap: Record<string, string> = { 'AAPL': '애플', 'TSLA': '테슬라', 'NVDA': '엔비디아', 'MSFT': '마이크로소프트' }
    try {
        const promises = symbols.map(async (symbol) => {
            try {
                ...
                const quoteData = await quoteResponse.json() as YahooFinanceChartResponse
                if (quoteData.chart?.result?.[0]) {
                    ...
                    let chartData: any[] = []
                    if (chartResponse.ok) {
                        const chartJson = await chartResponse.json() as YahooFinanceChartResponse
    ...
    } catch (error) { return c.json({ success: false, message: 'Failed to fetch US stocks data' }, 500) }
})

// 한국 주요 주식 4대장
stockRoutes.get('/api/kr-stocks/major', async (c) => {
    const symbols = ['005930.KS', '000660.KS', '373220.KS', '035420.KS']
    const nameMap: Record<string, string> = { '005930.KS': '삼성전자', '000660.KS': 'SK하이닉스', '373220.KS': 'LG에너지솔루션', '035420.KS': 'NAVER' }
    try {
        const promises = symbols.map(async (symbol) => {
            try {
                const quoteUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?interval=1d&range=1d`
        ...
        const results = (await Promise.all(promises)).filter(r => r !== null)
        return c.json({ success: true, count: results.length, stocks: results })
    } catch (error) { return c.json({ success: false, message: 'Failed to fetch KR stocks data' }, 500) }
})
...
stockRoutes.get('/api/macro-indicators', async (c) => {
    ...
    } catch (error) { return c.json({ success: false, message: 'Failed to fetch macro indicators data' }, 500) }
})
...
stockRoutes.get('/api/market-movers/:type', async (c) => {
    ...
    const screnerId = (screenerMap as any)[type]
    if (!screnerId) return c.json({ success: false, message: 'Invalid type' }, 400)
    try {
        const url = `https://query1.finance.yahoo.com/v1/finance/screener/predefined/saved?scrIds=${screnerId}&count=5`
        const response = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0' } })
        if (!response.ok) return c.json({ success: false, message: 'Failed to fetch data' }, 500)
        const data = await response.json() as YahooFinanceScreenerResponse
        if (data.finance?.result?.[0]?.quotes) {
            const stocks = data.finance.result[0].quotes.slice(0, 5).map((quote: YahooFinanceScreenerQuote) => ({
    ...
    } catch (error) { return c.json({ success: false, message: 'Failed to fetch market movers data' }, 500) }
})
...
stockRoutes.post('/api/poll/vote', async (c) => {
    ...
    if (!body.vote || !['bull', 'bear'].includes(body.vote)) return c.json({ success: false, message: 'Invalid vote' }, 400)
