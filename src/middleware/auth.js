...
export async function requireAdmin(c, next) {
    ...
    if (user.role !== 'admin') {
        console.log(`[Auth Debug] requireAdmin - Access Denied. User: ${user.email}, Role: '${user.role}'`);
        const acceptHeader = c.req.header('Accept') || '';
        if (acceptHeader.includes('application/json')) {
            return c.json({ success: false, message: '관리자 권한이 필요합니다', currentRole: user.role }, 403);
        }
...
export async function verifyPassword(password, hashedPassword) {
    ...
    console.log(`[Auth Debug] Input: ${password.substring(0, 1)}***, InputHash: ${hash}, DBHash: ${hashedPassword}`);
