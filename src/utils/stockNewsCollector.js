 ...
 */
export async function fetchStockNewsFromRSS(rssUrl) {
    try {
 ...
 * @returns 모든 주식 뉴스 배열
 */
export async function fetchMultipleRSS(rssUrls, delayMs = 1000) {
    const allNews = [];
 ...
 */
export function getStockNewsRSSUrls() {
    const queries = [
        '주식 특징주',
        '코스피 급등',
        '코스닥 급락',
    ...
    return queries.map(q => `https://news.google.com/rss/search?q=${encodeURIComponent(q)}&hl=ko&gl=KR&ceid=KR:ko`);
}
