...
export class MyPageController {
  ...
  static async addKeyword(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      ...
      logger.error('Failed to add keyword subscription', error as Error)
      throw error
    }
  }

  static async getKeywords(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to get keyword subscriptions', error as Error)
      throw error
    }
  }

  static async deleteKeyword(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to delete keyword subscription', error as Error)
      throw error
    }
  }
  ...
  static async addBookmark(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to add news bookmark', error as Error)
      throw error
    }
  }

  static async getBookmarks(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to get news bookmarks', error as Error)
      throw error
    }
  }

  static async deleteBookmark(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to delete news bookmark', error as Error)
      throw error
    }
  }
  ...
  static async getNewsByKeyword(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to get news by keyword', error as Error)
      throw error
    }
  }

  static async getNewsByMyKeywords(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      ...
      logger.error('Failed to get news by my keywords', error as Error)
      throw error
    }
  }
  ...
  static async markNewsAsRead(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to mark news as read', error as Error)
      throw error
    }
  }
  ...
  static async addWatchlistStock(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to add stock to watchlist', error as Error)
      throw error
    }
  }

  static async getWatchlistStocks(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to get watchlist stocks', error as Error)
      throw error
    }
  }

  static async updateWatchlistStock(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to update watchlist stock', error as Error)
      throw error
    }
  }

  static async deleteWatchlistStock(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to delete watchlist stock', error as Error)
      throw error
    }
  }

  static async addStockAlert(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to add stock alert', error as Error)
      throw error
    }
  }

  static async getStockAlerts(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to get stock alerts', error as Error)
      throw error
    }
  }

  static async deleteStockAlert(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to delete stock alert', error as Error)
      throw error
    }
  }

  static async getWatchlistStats(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to get watchlist stats', error as Error)
      throw error
    }
  }
  ...
  static async saveGameScore(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to save game score', error as Error)
      throw error
    }
  }

  static async getGameStats(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
  ...
  static async getGameHistory(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to get game history', error as Error)
      throw error
    }
  }

  static async getGameLeaderboard(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const gameType = c.req.query('game_type')
      if (!gameType) {
      ...
      logger.error('Failed to get game leaderboard', error as Error)
      throw error
    }
  }
  ...
  static async saveUtilSetting(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to save util setting', error as Error)
      throw error
    }
  }

  static async getUtilSettings(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to get util settings', error as Error)
      throw error
    }
  }

  static async saveUtilHistory(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to save util history', error as Error)
      throw error
    }
  }

  static async getUtilHistory(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to get util history', error as Error)
      throw error
    }
  }

  static async deleteUtilHistory(c: Context<{ Bindings: Bindings; Variables: Variables }>) {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Unauthorized', 401)
      ...
      logger.error('Failed to delete util history', error as Error)
      throw error
    }
  }
}
