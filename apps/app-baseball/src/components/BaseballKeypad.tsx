    ...
    <div className="w-full flex flex-col gap-1.5">
      ...
      <div className="flex items-center justify-between px-2.5 py-1.5 bg-[#f0f4f8] rounded-2xl shadow-[inset_3px_3px_6px_#d1d9e6,inset_-3px_-3px_6px_#ffffff] border border-white/60">
        <div className="flex items-center gap-2">
          <span className="text-[11px] font-black text-slate-500 uppercase tracking-tight">선택:</span>
              ...
              return (
                <div
                  ...
                  className={`w-8 h-8 rounded-lg flex items-center justify-center font-mono text-base font-black transition-all ${
                    isFilled
                      ? 'bg-gradient-to-b from-amber-400 to-amber-500 text-slate-900 shadow-[2px_2px_4px_#cbd5e1,-1px_-1px_3px_#ffffff] scale-105'
        ...
        <button
          ...
          className={`flex items-center gap-1 px-3.5 py-1.5 rounded-xl font-black text-xs tracking-tight transition-all transform active:scale-95 ${
            isReadyToPitch
              ? 'bg-gradient-to-r from-red-500 to-rose-600 text-white shadow-[0_3px_10px_rgba(239,68,68,0.4)] hover:brightness-105 animate-pulse cursor-pointer'
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => {
          ...
          return (
            <button
              ...
              className={`h-9.5 rounded-xl font-black text-sm transition-all flex items-center justify-center ${
                isSelected
                  ? 'bg-slate-200/60 text-slate-300 shadow-[inset_2px_2px_4px_#cbd5e1] cursor-not-allowed'
                  : disabled
                  ? 'bg-[#f0f4f8] text-slate-300 cursor-not-allowed'
                  : 'bg-[#f0f4f8] text-slate-700 shadow-[3px_3px_6px_#d1d9e6,-3px_-3px_6px_#ffffff] hover:shadow-[1px_1px_3px_#d1d9e6,-1px_-1px_3px_#ffffff] active:shadow-[inset_2px_2px_4px_#d1d9e6,inset_-2px_-2px_4px_#ffffff] active:scale-95 cursor-pointer'
              }`}
            >
              {num}
            </button>
        ...
        <button
          ...
          disabled={disabled || inputDigits.length === 0}
          className="h-9.5 rounded-xl font-black text-[11px] transition-all flex items-center justify-center gap-0.5 bg-[#f0f4f8] text-rose-500 shadow-[3px_3px_6px_#d1d9e6,-3px_-3px_6px_#ffffff] active:shadow-[inset_2px_2px_4px_#d1d9e6,inset_-2px_-2px_4px_#ffffff] disabled:opacity-40 disabled:cursor-not-allowed active:scale-95 cursor-pointer"
        >
