...
const GameBoard = ({ board, currentPiece }) => {
    ...
    return (_jsx("div", { className: "bg-slate-900 border-4 border-slate-700 rounded-xl p-2 w-fit mx-auto shadow-2xl overflow-hidden shadow-emerald-900/40", children: _jsx("div", { className: "grid gap-[1px] bg-slate-800", style: {
                gridTemplateColumns: `repeat(${board[0].length}, 1fr)`,
                gridTemplateRows: `repeat(${board.length}, 1fr)`,
                width: '240px',
                height: '480px'
            }, children: renderedBoard.map((row, rIdx) => (row.map((cell, cIdx) => (_jsx("div", { className: `w-full h-full border  
