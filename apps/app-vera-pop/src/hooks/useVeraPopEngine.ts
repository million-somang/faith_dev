...
export const GEM_COLORS: Record<GemType, { name: string; hex: string; gradient: string; glow: string }> = {
    0: { name: '사파이어 블루', hex: '#3B82F6', gradient: 'from-blue-400 to-blue-600', glow: 'rgba(59, 130, 246, 0.7)' },
    1: { name: '에메랄드 그린', hex: '#10B981', gradient: 'from-emerald-400 to-emerald-600', glow: 'rgba(16, 185, 129, 0.7)' },
    2: { name: '아메지스트 퍼플', hex: '#8B5CF6', gradient: 'from-purple-400 to-purple-600', glow: 'rgba(139, 92, 246, 0.7)' },
    3: { name: '토파즈 옐로우', hex: '#F59E0B', gradient: 'from-amber-400 to-amber-600', glow: 'rgba(245, 158, 11, 0.7)' },
    4: { name: '루비 레드', hex: '#EF4444', gradient: 'from-rose-400 to-red-600', glow: 'rgba(239, 68, 68, 0.7)' },
};

let nextId = 1;
const createGem = (row: number, col: number, type?: GemType, special: SpecialType = 'none'): Gem => ({
    id: `gem_${nextId++}`,
...
export function useVeraPopEngine(options: EngineOptions = { isActive: true }) {
    ...
    // 연쇄 폭발 및 드롭 처리
    const processMatches = useCallback(async (initialBoard: Gem[][], triggerSpecial?: { r: number; c: number; special: SpecialType }) => {
        ...
        while (!isGameOverRef.current) {
            ...
            if (matches.length === 0 && !triggerSpecial) break;
            ...
            matches.forEach(({ r, c }) => destroyed.add(`${r},${c}`));
            ...
            addedScore = destroyed.size * 100 * cascadeCombo * feverMultiplier;
            ...
            setCombo(prev => {
                const next = timeDiff < 2000 ? prev + 1 : 1;
            ...
            newSpecialsToSpawn.forEach(sp => {
                droppedBoard[sp.r][sp.c] = createGem(sp.r, sp.c, sp.type, sp.special);
            });
    ...
    const swapGems = useCallback(async (r1: number, c1: number, r2: number, c2: number) => {
        ...
        nextBoard[r1][c1] = { ...g2, row: r1, col: c1 };
        nextBoard[r2][c2] = { ...g1, row: r2, col: c2 };

        const { matches } = findMatches(nextBoard);

        if (matches.length > 0) {
            setBoard(nextBoard);
            await processMatches(nextBoard);
        } else {
            // 매칭 실패 시 원위치
            setBoard(nextBoard);
            setTimeout(() => setBoard(board), 180);
        }
