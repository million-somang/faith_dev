...
// 헬퍼 함수
async function logActivity(db: any, userId: number | string | null, action: string, description: string, ip?: string) {
    try {
...
adminRoutes.get('/api/admin/users', requireAdmin, async (c) => {
    ...
    try {
        ...
        const bindings = []
        if (search) { query += ' AND (email LIKE ? OR name LIKE ?)'; bindings.push(`%${search}%`, `%${search}%`) }
        if (level) { query += ' AND level = ?'; bindings.push(parseInt(level)) }
...
adminRoutes.get('/api/admin/users/:id', requireAdmin, async (c) => {
    ...
    try {
        ...
        if (!user) return c.json({ success: false, message: '회원을 찾을 수 없습니다.' }, 404)
        return c.json({ success: true, user })
    } catch (error) { return c.json({ success: false, message: '서버 오류' }, 500) }
})
...
adminRoutes.put('/api/admin/users/:id', requireAdmin, async (c) => {
    ...
    try {
        ...
        const targetUser = await DB.prepare('SELECT email FROM users WHERE id = ?').bind(targetUserId).first() as any
        ...
        await logActivity(DB, userId as string, 'admin_action', `회원 정보 수정: ${targetUser?.email}`)
        return c.json({ success: true, message: '회원 정보가 수정되었습니다.' })
    } catch (error) {
        console.error('Update User Error:', error);
        return c.json({ success: false, message: '서버 오류', error: String(error) }, 500)
    }
})
...
adminRoutes.patch('/api/admin/users/:id/status', requireAdmin, async (c) => {
    ...
    try {
        ...
        if (!['active', 'suspended'].includes(status)) return c.json({ success: false, message: '올바르지 않은 상태' }, 400)
        const targetUser = await DB.prepare('SELECT email, name FROM users WHERE id = ?').bind(targetUserId).first() as any
        await DB.prepare('UPDATE users SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').bind(status, targetUserId).run()
        await logActivity(DB, userId as string, 'admin_action', `회원 상태 변경: ${targetUser?.email} → ${status}`)
        if (status === 'suspended') {
...
adminRoutes.delete('/api/admin/users/:id', requireAdmin, async (c) => {
    ...
    try {
        ...
        await logActivity(DB, userId as string, 'admin_action', `회원 삭제: ${targetUser?.email}`)
...
adminRoutes.get('/api/admin/activity-logs', requireAdmin, async (c) => {
    ...
    try {
        ...
        const bindings = []
        if (action) { query += ' AND al.action = ?'; bindings.push(action) }
        query += ' ORDER BY al.created_at DESC LIMIT ?'; bindings.push(limit)
...
adminRoutes.get('/api/admin/notifications', requireAdmin, async (c) => {
    ...
    try {
        ...
        const unreadCount = await DB.prepare('SELECT COUNT(*) as count FROM notifications WHERE (target_user_id IS NULL OR target_user_id = ?) AND is_read = 0').bind(userId).first() as any
...
adminRoutes.post('/api/admin/users/batch', requireAdmin, async (c) => {
    ...
    try {
        ...
        if (!action || !userIds || !Array.isArray(userIds)) return c.json({ success: false, message: '올바르지 않은 요청' }, 400)
        let query = '', bindings: any[] = []
        switch (action) {
            case 'change_level': query = `UPDATE users SET level = ?, updated_at = CURRENT_TIMESTAMP WHERE id IN (${userIds.map(() => '?').join(',')})`; bindings = [value, ...userIds]; break
            case 'change_status': query = `UPDATE users SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id IN (${userIds.map(() => '?').join(',')})`; bindings = [value, ...userIds]; break
            case 'delete': query = `UPDATE users SET status = 'deleted', updated_at = CURRENT_TIMESTAMP WHERE id IN (${userIds.map(() => '?').join(',')})`; bindings = userIds; break
            default: return c.json({ success: false, message: '올바르지 않은 작업' }, 400)
        }
        await DB.prepare(query).bind(...bindings).run()
        await logActivity(DB, userId as string, 'admin_action', `일괄 처리: ${action} (${userIds.length}명)`)
