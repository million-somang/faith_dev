...
// 헬퍼: 테이블 동적 초기화 (comboy_saves_v2)
async function initComboyTable(DB: any) {
    await DB.prepare(`
        CREATE TABLE IF NOT EXISTS comboy_saves_v2 (
...
comboyRoutes.get('/api/comboy/list', requireAuth, async (c) => {
    const DB = getDB(c);
    ...
    try {
        await initComboyTable(DB);

        const results = await DB.prepare(`
            SELECT slot_no, slot_name, updated_at
            FROM comboy_saves_v2
        ...
        console.error('[Comboy List Error]', error.message, error.stack);
        return c.json({ success: false, error: { code: 500, message: 'Internal Server Error: ' + error.message } }, 500);
    }
});

// 2. 세이브 데이터 저장 (POST /api/comboy/save)
comboyRoutes.post('/api/comboy/save', bodyLimit({ maxSize: 20 * 1024 * 1024 }), requireAuth, async (c) => {
    ...
    try {
        ...
        console.log('[Comboy] Saving slot:', parsedSlotNo, 'name:', slotName, 'for user:', user.id, 'game:', normalized);
...
// 3. 특정 슬롯 세이브 데이터 로드 (GET /api/comboy/load)
comboyRoutes.get('/api/comboy/load', requireAuth, async (c) => {
    ...
    try {
        ...
        console.log('[Comboy] Loading slot:', slotNo, 'for user:', user.id, 'game:', normalized);
...
// 4. 특정 슬롯 세이브 데이터 삭제 (DELETE /api/comboy/delete)
comboyRoutes.delete('/api/comboy/delete', requireAuth, async (c) => {
    const DB = getDB(c);
    const user = c.get('user');
    if (!user) {
        return c.json({ success: false, error: { code: 401, message: 'Unauthorized' } }, 401);
    ...
    try {
        ...
        console.log('[Comboy] Deleting slot:', slotNo, 'for user:', user.id, 'game:', normalized);

        await DB.prepare(`
            DELETE FROM comboy_saves_v2
            WHERE user_id = ? AND game_name = ? AND slot_no = ?
        ...
        console.error('[Comboy Delete Error]', error.message, error.stack);
        return c.json({ success: false, error: { code: 500, message: 'Internal Server Error: ' + error.message } }, 500);
    }
});
