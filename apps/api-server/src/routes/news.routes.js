...
import { findRelatedStocks } from '@faithportal/core-utils';
...
news.get('/api/news', async (c) => {
    ...
    try {
        ...
        if (includeStocks) {
            const newsWithStocks = await Promise.all(newsItems.map(async (n) => {
...
news.post('/api/news/:id/vote', requireAuth, async (c) => {
    ...
    try {
        const existingVote = await pool.query('SELECT * FROM news_votes WHERE news_id = $1 AND user_id = $2', [id, user.id]);
        ...
        // New vote
        await pool.query('INSERT INTO news_votes (news_id, user_id, vote_type) VALUES ($1, $2, $3)', [id, user.id, type]);
        const field = type === 'up' ? 'vote_up' : 'vote_down';
        await pool.query(`UPDATE news SET ${field} = ${field} + 1 WHERE id = $1`, [id]);
...
news.post('/api/news/:id/summarize', async (c) => {
    ...
    try {
        ...
        await pool.query('UPDATE news SET ai_summary = $1, sentiment = $2, ai_processed = TRUE WHERE id = $3', [aiSummary, sentiment, id]);
